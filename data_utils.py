...
def prep_all_data(file_dict, batch_size=1, device="cuda"):
    ...
    train_data = Batch(df_train, text2idx, label2idx)
    val_data = Batch(df_val, text2idx, label2idx)
...
class Batch(object):
    ...
    def __iter__(self):
        ...
        for i in idx:
            sample = Sample(i, self.df.loc[i], self.t2i, self.l2i)
            yield sample


class Sample(object):
    def __init__(self, sample_id, data, text2idx, label2idx):
        self.raw_text = data.utterances
        self.raw_label = data["IOB Slot tags"]
        self.t2i = text2idx
        self.l2i = label2idx
        self.sample_id = sample_id
        self.text = torch.tensor(self.convert_text())
        self.label = torch.tensor(self.convert_label())

    def convert_text(self):
        self.text = []
        for t in self.raw_text.lower().split(" "):
            try:
